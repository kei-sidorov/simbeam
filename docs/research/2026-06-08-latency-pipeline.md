...
## Сессия 2026-07-19 — стартовая латентность attach → первый кадр (cold-start)

Мотивация: на реальном iOS-клиенте между «выбрал симулятор / первый тап» и первым
кадром на экране наблюдается **~5 с**. Всё, что выше по доке, — про *установившийся*
режим (g2g ≈ 0.4–0.5 с); стартовая латентность — отдельная величина, и её никто
раньше не раскладывал. Здесь разложена **серверная часть** цепочки `sim.Attach`.

Конфигурация: тот же Mac (12 ядер), симулятор iPhone 17 Pro (iOS 26.1, 1206×2622
ретина), scale 0.5, 8 Мбит/с, 15 fps, ffmpeg `h264_videotoolbox`. Замер — одноразовым
пробником, который повторяет ровно тело `sim.Backend.Attach`
(`idb.Spawn` → `Describe` → `ScreenshotStream` → `encoder.Encode`) с таймстампами по
стадиям. Пробник (`cmd/latbench`) после сессии удалён.

### Раскладка (3 прогона, разброс мал)

| Стадия | Время |
|---|---|
| `idb.Spawn` — старт процесса `idb_companion` + первый успешный `Describe` (readiness) | **1.21 с** (константа: 1.213 / 1.214 / 1.216) |
| второй `Describe` (в `Attach` после readiness) | ~0 мс |
| первый (холодный) `Screenshot` RPC — коннект к framebuffer | +170–195 мс |
| **прогрев ffmpeg / videotoolbox** — VT глотает ~8–10 ретина-PNG (по ~2.3 МБ) прежде чем выдать первый AU | **+780–840 мс** |
| **ИТОГО attach → первый AU из ffmpeg** | **≈ 2.0 с** (2.030 / 1.996 / 1.995) |

Первый AU — это IDR (~43 КБ), дальше AU идут пачкой ~73 мс (frameTimer догоняет).

### Выводы (факты, не задачи)

- Серверный cold-start = **~2.0 с**, из которых **~60% — это `idb.Spawn`** (1.21 с,
  дубовая константа) и **~40% — прогрев videotoolbox** (~0.8 с). Холодный screenshot
  и describe пренебрежимо малы.
- Свежий sidecar спавнится **на каждый `Attach`** (см. `sim.Attach` → `idb.Spawn`),
  т.е. эти ~2 с платятся заново и на смену качества (`doQuality` = detach+attach), и
  на реконнект. Пул/пре-варм sidecar’а на выбранный симулятор срезал бы 1.21 с.
- Прогрев VT «глотает» ~8–10 кадров — это тот самый стартовый артефакт в счётчике
  in-flight из секции 2026-07-17 (out=0 первые ~3.5 с), теперь измеренный как ~0.8 с
  реальной задержки первого AU (остальное — заполнение конвейера кадрами по 66.7 мс).
- **Наблюдаемые на iPad ~5 с serverом не объясняются** (~2.0 с). Оставшиеся ~3 с —
  вне этого замера: установление WebRTC (ICE/DTLS/rendezvous через брокер) до attach
  и/или приёмная сторона (когда у клиента реально готов декодер к моменту прихода
  первого IDR). Отдельная сессия; здесь не мерили.
