...
**Goal:** Ship simcast as a forkable open-core project — release the macOS daemon via a Homebrew tap and the Linux signaller as a binary, and stand up a VPS that auto-updates `simcast-signal` from GitHub Releases with zero server secrets in the repo.

**Architecture:** One GoReleaser pipeline builds both binaries on a git tag and publishes a GitHub Release + a Homebrew formula (separate public tap). The VPS runs `simcast-signal` + coturn behind Caddy (auto-TLS); a systemd timer runs a pull-updater that swaps the binary when a newer release appears. Generic deploy scaffolding lives in the repo; secrets/personal values live only on the server in `/etc/simcast/signal.env`.
...
- Modify `cmd/simcast-signal/main.go` — `version` var + `--version` flag.
- Modify `cmd/simcastd/main.go` — `version` var + `version` subcommand.
- Create `.github/workflows/ci.yml` — fmt/vet/test/build on push & PR.
- Create `.github/workflows/release.yml` — GoReleaser on tag `v*`.
- Create `deploy/systemd/simcast-signal.service` — broker unit.
- Create `deploy/systemd/simcast-signal-update.service` + `.timer` — pull-updater unit + schedule.
- Create `deploy/simcast-signal-update.sh` — pull-updater script.
...
- [ ] **Step 1: Add version var + flag to `simcast-signal`**

In `cmd/simcast-signal/main.go`, add a package-level var after the imports block (before `func main`):
...
- [ ] **Step 2: Add version subcommand to `simcastd`**

In `cmd/simcastd/main.go`, add after the imports block (before `func main`):
...
Run:
```bash
go build ./... && go run ./cmd/simcast-signal --version && go run ./cmd/simcastd version
```
...
Run:
```bash
go run -ldflags "-X main.version=9.9.9" ./cmd/simcast-signal --version
```
...
```bash
git add cmd/simcast-signal/main.go cmd/simcastd/main.go
git commit -m "feat(cmd): version flag for simcast-signal and simcastd
...
GoReleaser builds darwin binaries for `simcastd` and a linux binary for `simcast-signal`, all pure-Go (`CGO_ENABLED=0`; the sqlite driver is `modernc.org/sqlite`, decision #61). It also generates the Homebrew formula and pushes it to the `homebrew-simcast` tap.
...
builds:
  ...
  - id: simcast-signal
    main: ./cmd/simcast-signal
    binary: simcast-signal
    env:
      - CGO_ENABLED=0
    goos: [linux]
    goarch: [amd64]
...
archives:
  - id: simcastd
    ids: [simcastd]
    name_template: "simcastd_{{ .Version }}_{{ .Os }}_{{ .Arch }}"
  - id: simcast-signal
    ids: [simcast-signal]
    name_template: "simcast-signal_{{ .Version }}_{{ .Os }}_{{ .Arch }}"
...
brews:
  - name: simcastd
    ...
    repository:
      owner: kei-sidorov
      name: homebrew-simcast
      token: "{{ .Env.HOMEBREW_TAP_TOKEN }}"
    homepage: "https://github.com/kei-sidorov/simcast"
    description: "simcast daemon — stream an iOS simulator from Mac to iPad"
    dependencies:
      - name: idb-companion
      - name: ffmpeg
    caveats: |
      simcastd is shipped as an unsigned binary. Homebrew removes the macOS
...
```
Expected: completes successfully and produces `dist/` containing `simcastd` darwin arm64+amd64 archives, a `simcast-signal` linux amd64 archive, and `checksums.txt`. (The `brews` step is skipped automatically in snapshot mode.)

- [ ] **Step 4: Confirm the injected version**

Run:
```bash
tar -xzf dist/simcast-signal_*_linux_amd64.tar.gz -O simcast-signal > /tmp/scs && chmod +x /tmp/scs
# Linux binary won't run on macOS; instead confirm the version string is embedded:
strings dist/simcastd_*_darwin_arm64/simcastd 2>/dev/null | grep -E "^[0-9]+\.[0-9]+\.[0-9]+" | head -1 || echo "snapshot version embedded"
```
...
git commit -m "build: GoReleaser config for simcastd (Homebrew) + simcast-signal
...
**Files:**
- Create: `deploy/systemd/simcast-signal.service`
- Create: `deploy/systemd/simcast-signal-update.service`
- Create: `deploy/systemd/simcast-signal-update.timer`
- Create: `deploy/signal.env.example`

The broker binds to localhost (Caddy fronts it with TLS). Flags + secrets come from `/etc/simcast/signal.env` (NOT committed; the `.example` is the template). `$SIMCAST_SIGNAL_ARGS` is word-split by systemd; `SIMCAST_APP_SECRET` is read from the environment by the binary.

- [ ] **Step 1: Write `deploy/systemd/simcast-signal.service`**

```ini
[Unit]
Description=simcast signaling broker
...
User=simcast
Group=simcast
EnvironmentFile=/etc/simcast/signal.env
ExecStart=/usr/local/bin/simcast-signal $SIMCAST_SIGNAL_ARGS
Restart=always
RestartSec=2
...
- [ ] **Step 2: Write `deploy/systemd/simcast-signal-update.service`**

```ini
[Unit]
Description=simcast-signal auto-update (pull latest GitHub release)
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/usr/local/bin/simcast-signal-update.sh
```

- [ ] **Step 3: Write `deploy/systemd/simcast-signal-update.timer`**

```ini
[Unit]
Description=Periodically check for a new simcast-signal release
...
```bash
# Copy to /etc/simcast/signal.env on the server and fill in real values (chmod 600).
...
SIMCAST_SIGNAL_ARGS=--addr 127.0.0.1:9000 --db /var/lib/simcast/simcast.db --stun stun:stun.l.google.com:19302 --turn turn:YOUR_DOMAIN:3478 --turn-secret change-me-same-as-coturn --turn-ttl 1m
```
...
```bash
ls deploy/systemd/*.service deploy/systemd/*.timer && grep -q "ExecStart=/usr/local/bin/simcast-signal " deploy/systemd/simcast-signal.service && echo UNITS_OK
```
...
**Files:**
- Create: `deploy/simcast-signal-update.sh`

The script must match the GoReleaser archive name (`simcast-signal_<version>_linux_amd64.tar.gz`) and the version reported by `simcast-signal --version` (GoReleaser `.Version` has no leading `v`, matching `--version` output).

- [ ] **Step 1: Write `deploy/simcast-signal-update.sh`**
...
Run:
```bash
chmod +x deploy/simcast-signal-update.sh
which shellcheck || brew install shellcheck
shellcheck deploy/simcast-signal-update.sh && echo SHELLCHECK_OK
```
...
Run:
```bash
SIMCAST_REPO=kei-sidorov/simcast SIMCAST_BIN=/nonexistent bash deploy/simcast-signal-update.sh --dry-run || true
```
...
```bash
git add deploy/simcast-signal-update.sh
git commit -m "deploy: pull-based simcast-signal auto-updater (checksum-verified, atomic)
...
```
# MUST equal the broker's --turn-secret (see /etc/simcast/signal.env).
...
| 67 | Phase 4: macOS-демон — **Homebrew tap `kei-sidorov/homebrew-simcast`, предсобранный неподписанный бинарь** (без Developer ID / нотаризации); зависимости `idb-companion` + `ffmpeg`. Обновление `brew upgrade` (без автообновления на Mac) | аудитория = iOS-разработчики, Homebrew снимает quarantine → unsigned обычно запускается; подпись/нотаризацию добавим, если Gatekeeper прижмёт |
...
Run:
```bash
gh repo create kei-sidorov/homebrew-simcast --public --description "Homebrew tap for simcast"
```
Expected: empty tap repo exists. GoReleaser will push the formula into it on first release.

- [ ] **Step 4: Add the tap token secret**

Create a fine-grained PAT with `contents: write` on `kei-sidorov/homebrew-simcast`, then:
```bash
gh secret set HOMEBREW_TAP_TOKEN --repo kei-sidorov/simcast
```
Expected: secret stored. (The default `GITHUB_TOKEN` cannot push to a different repo, hence this PAT.)

- [ ] **Step 5: Verify CI ran**

Run: `gh run list --repo kei-sidorov/simcast --workflow ci.yml`
...
three archives + `checksums.txt`, and pushes the `simcastd` formula to the tap.

- [ ] **Step 3: Verify the release artifacts**

Run:
```bash
gh release view v0.1.0 --repo kei-sidorov/simcast
gh api repos/kei-sidorov/homebrew-simcast/contents/Formula/simcastd.rb >/dev/null && echo FORMULA_OK
```
...
- **Name consistency:** archive name `simcast-signal_<version>_linux_amd64.tar.gz` is
...
- **Secrets boundary:** only `HOMEBREW_TAP_TOKEN` lives in repo Actions secrets (Task 9);
  no server/SSH secrets anywhere; personal values only in on-server `/etc/simcast/signal.env`.
```
