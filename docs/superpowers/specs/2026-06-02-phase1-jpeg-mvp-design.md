...
Browser (debug-клиент, web/debug/index.html — отдельный файл)
  │  GET  /api/simulators ─────────► simcastd ──► companion.List()        (CLI --list 1)
  │  POST /api/boot {udid} ────────► simcastd ──► companion.Boot(udid)    (CLI --boot)
  │  WS   /session?udid=X ─────────► simcastd
...
└── cmd/simcastd/main.go         # list (есть) + serve [--addr :8080] [--web ./web/debug]
```
...
- `make run` (или `go run ./cmd/simcastd serve --web ./web/debug`) → открыть браузер →
...
- `make build` — `go build ./...`.
- `make run` — `go run ./cmd/simcastd serve --web ./web/debug`.
- `make test` — `go test ./...`.
