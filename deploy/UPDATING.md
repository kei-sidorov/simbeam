...
- `OnCalendar=*:0/10` — проверка каждые 10 минут по стенным часам (:00, :10, …);
- `Persistent=true` — тик, пропущенный из-за выключенного сервера, догоняется при
  старте (для `OnCalendar` это работает; для монотонных таймеров игнорируется).

> **Грабли, на которые уже наступили (2026-08-09).** Раньше таймер был монотонный:
> `OnBootSec=2min` + `OnUnitActiveSec=10min`. `OnBootSec` срабатывает один раз за
> загрузку, а `OnUnitActiveSec` отсчитывается от последней активации *сервиса* —
> поэтому если таймер остановить (а руководство само велит это делать перед
> релизом, меняющим флаги брокера) и потом запустить, обе точки оказываются в
> прошлом: `systemctl is-active` показывает `active`, а `NextElapse` —
> `infinity`. Таймер молча не тикает вообще. Так брокер восемь часов не проверял
> обновления, и v0.16.0 пришлось ставить руками. `OnCalendar` этой дыры не имеет:
> следующая точка есть всегда, независимо от того, как таймер был запущен.
> Проверить здоровье: `systemctl list-timers simbeam-signal-update.timer` — в
> колонке `NEXT` должна быть дата, а не прочерк.
