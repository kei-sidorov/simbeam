...
видео и управление получает лишь спаренный клиент. Незащищённого локального стрима нет —
старые `WS /session` (JPEG-поллинг) и dev `WS /rtc` (offer/answer без auth) удалены
(`docs/decisions.md` №75). Как поднять брокер, демон и спарить браузер — см.
[Удалёнка / рандеву](#удалёнка--рандеву-phase-3b) и [Remote pairing](#remote-pairing-phase-3c).

Демон без `--web` отдаёт только management-API (стрима тут нет):
