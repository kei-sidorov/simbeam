...
simcast is a thin orchestration layer, not a new capture engine. All the heavy lifting —
...
iPad / browser                                Mac
┌──────────────┐                       ┌───────────────────────────────┐
│  <video>     │ ◄── H.264 (WebRTC) ── │ simcastd (Go)                 │
...
| Part | What it is |
|------|------------|
| **`simcastd`** | macOS daemon. Boots/streams a simulator, serves video over WebRTC, injects input. Open source (this repo). |
| **`simcast-signal`** | Reference signalling broker. Rendezvous over WSS, relays the SDP handshake, issues short-lived TURN credentials. Open source (this repo). |
...
simcast is **open-core**: the server is OSS, the polished client is the commercial product.
...
```bash
brew install --cask kei-sidorov/simcast/simcastd
simcastd version
```

Update later with `brew upgrade --cask simcastd`.
...
Pair a device and start streaming. By default `simcastd serve` connects to the **public
...
with `simcastd unpair <clientPubKey>`. The daemon's identity lives in `~/.simcast/` (0600).
...
```bash
simcastd serve --signal wss://your-broker.example/ws
```
...
```bash
go run ./cmd/simcastd list                      # enumerate simulators
make run-remote                                 # daemon + baked broker + debug web client
go run ./cmd/simcast-signal --addr :9000        # run a local broker
```
...
```
simcast/
├── cmd/simcastd/         # macOS daemon (boot, stream, input, pairing)
├── cmd/simcast-signal/   # reference signalling broker
